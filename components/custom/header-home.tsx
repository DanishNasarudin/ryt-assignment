...
export default function HeaderHome() {
  ...
  return (
    <BlurView intensity={50} experimentalBlurMethod="dimezisBlurView">
      <SafeAreaView className="h-0 pb-4 border-border border-b-[1px]">
        <ThemedView className="!bg-transparent px-5 flex flex-row justify-between gap-4 w-screen">
          <TouchableOpacity
            className="!text-accent bg-background border-border border-[1px] w-10 h-10 flex justify-center items-center rounded-full"
          ...
          <TouchableOpacity>
            {userExist ? (
              ...
              <TouchableOpacity className="w-10 h-10 rounded-full bg-background border-border border-[1px] flex justify-center items-center">
                <IconSymbol
                  name="person"
                  color={tailwindToHex("accent", theme)}
                  size={16}
